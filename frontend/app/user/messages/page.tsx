...
function MessagesPageContent() {
  ...
  const [showLogoutConfirm, setShowLogoutConfirm] = useState(false); // New state for Header
  const messagesEndRef = useRef<HTMLDivElement>(null);
  
  // Destructure needed values from auth hook
  const { user, isLoading } = useAuth({ redirect: false });

  // Header Props Logic
  const isAuthenticated = !!user;
  const userEmail = user?.email || "";
  const profileImageUrl = user?.profile_image || undefined;
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24"> {/* Adjusted py-24 for Fixed Header */}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <p className="text-sm font-semibold text-[#111827] truncate">{conversation.user.name}</p>
                        <span className="text-xs text-[#9ca3af] whitespace-nowrap">{formatTime(conversation.updated_at)}</span>
                      </div>
                      <p className="text-xs text-[#6b7280] truncate">
                        {conversation.latest_message.type === 'file' ? `📎 ${conversation.latest_message.content}` : conversation.latest_message.content}
                      </p>
                    </div>
                  </button>
                ))
              ...
              <>
                <div className="px-6 py-4 border-b border-[#f0f2f5] flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-[#111827]">{selectedConversation.user.name}</p>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4 bg-[#f9fafb]">
                  {messages.map((message) => (
                    <div key={message.id} className={`flex ${message.sender_id === user?.id ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-xl ${message.sender_id === user?.id ? 'order-2' : 'order-1'}`}>
                        <div className={`rounded-2xl px-4 py-3 text-sm shadow-sm ${
                          message.sender_id === user?.id ? 'bg-[#7EB0AB] text-white rounded-tr-none' : 'bg-white border border-[#e5e7eb] rounded-tl-none text-[#111827]'
                        }`}>
                          {message.type === 'file' ? (
                            <div>
                              <div className="flex items-center gap-2"><span>📎</span><span>{message.content}</span></div>
                              {message.file_path && <a href={`http://127.0.0.1:8000/storage/${message.file_path}`} target="_blank" className="text-xs underline mt-1 block">View File</a>}
                            </div>
                          ) : message.type === 'image' ? (
                            <div>
                              <Image src={`http://127.0.0.1:8000/storage/${message.file_path}`} alt="Shared" width={300} height={200} className="max-w-full rounded-lg mb-1" />
                              <span>{message.content}</span>
                            </div>
                          ) : message.content}
                        </div>
                        <p className={`mt-1 text-xs text-[#9ca3af] ${message.sender_id === user?.id ? 'text-right' : 'text-left'}`}>{formatTime(message.created_at)}</p>
                      </div>
                    </div>
                  ))}
                ...
                <div className="border-t border-[#e5e7eb] bg-white px-6 py-4">
                  {selectedFile && (
                    <div className="mb-3 p-2 bg-[#f0f9ff] border border-[#bae6fd] rounded-lg flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span>📎</span><span className="text-sm text-[#0c4a6e]">{selectedFile.name}</span>
                      </div>
                      <button onClick={() => { setSelectedFile(null); if (fileInputRef) fileInputRef.value = ''; }} className="text-[#ef4444]">✕</button>
                    </div>
                  )}
                  <div className="flex items-center gap-3">
                    <input type="file" ref={(ref) => setFileInputRef(ref)} className="hidden" accept="image/*,.pdf,.doc,.docx" onChange={handleFileSelect} />
                    <button onClick={() => fileInputRef?.click()} className="p-2 text-[#6b7280]">📎</button>
                    <input
                      type="text"
                      value={newMessage}
                      ...
                      className="flex-1 rounded-full border border-[#e5e7eb] bg-[#f9fafb] px-4 py-2 text-sm focus:ring-[#7EB0AB] focus:outline-none"
                    />
                    <button onClick={sendMessage} disabled={sendingMessage || (!newMessage.trim() && !selectedFile)} className="p-2 rounded-full bg-[#7EB0AB] text-white">
                      {sendingMessage ? '⏳' : '➤'}
                    </button>
                  </div>
                </div>
              ...
              <div className="flex-1 flex items-center justify-center text-center">
                <div>
                  <div className="w-16 h-16 bg-[#e5e7eb] rounded-full flex items-center justify-center mx-auto mb-4">
                    <svg className="w-8 h-8 text-[#9ca3af]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
                  </div>
